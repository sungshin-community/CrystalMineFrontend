...
export const getPinnedBoardList = async () => {
    ...
    try {
        ...
        const officialResponse = await client.get<AxiosResponse<Board[]>>(`/boards/pin?type=1`, {
            headers: {
        ...
        const customResponse = await client.get<AxiosResponse<Board[]>>(`/boards/pin?type=0`, {
            headers: {
                Authorization: `Bearer ${accessToken}`
            }
        });
...
export const getOfficialBoardList = async () => {
    try {
        ...
        const response = await client.get<AxiosResponse<Board[]>>(`/boards?${params}`, {
            headers: {
                Authorization: `Bearer ${accessToken}`
            }
        });
...
export const toggleBoardPin = async (boardId: number) => {
    try {
        const accessToken = await AsyncStorage.getItem("accessToken");
        const response = await client.post<AxiosResponse>(`/boards/${boardId}/pin`, null, {
            headers: {
