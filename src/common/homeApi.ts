...
export const getPinBoardContents = async () => {
  try {
    const response = await client.get<AxiosResponse<PinBoardDto[]>>(
      '/home/pin-boards',
    );
    return response;
  } catch (e) {
...
export const getHotBoardContents = async () => {
  try {
    const response = await client.get<AxiosResponse<HotBoardDto>>(
      '/home/hot-posts',
    );
    return response;
  } catch (e) {
...
export async function getNewPosts() {
  try {
    const response = await client.get('/home/new-posts');
    console.log('방금 올라온 글: ', response);
    return response.data.data;
  } catch (error) {
    console.error('방금 올라온 글 오류 발생: ', error);
    return [];
  }
}
