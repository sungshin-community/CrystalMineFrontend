...
import {searchMyPosts} from '../../common/SearchApi';
...
export default function MyPostList({searchWord}: Props) {
  ...
  useEffect(() => {
    async function init() {
      setIsLoading(true);
      const response = await searchMyPosts(searchWord, 0, sortBy);
      setCurrentPage(0);
  ...
  const fetchNextPage = async () => {
    ...
    const response = await searchMyPosts(searchWord, currentPage + 1, sortBy);
