...
type RootStackParamList = {
  ...
  MyCommentSearchResult: {searchWord: string};
};
...
function MyCommentSearch({navigation, route}: Props) {
  ...
  const saveRecentSearchWords = async (text: string) => {
    try {
      ...
      if (true) {
        await AsyncStorage.setItem('recentMyCommentSearch', JSON.stringify(tempArray));
      }
    } catch (error) {
      console.error('최근 검색어 가져오기 실패', error);
    }
        ...
        <View style={{flex: 1, marginTop: 14}}>
          {
            recentSearchWords?.map((text, index) => (
              <TouchableOpacity
                ...
                key={index}
                onPress={() => navigation.navigate('MyCommentSearchResult', {searchWord: text})}
              >
...
export default MyCommentSearch;
