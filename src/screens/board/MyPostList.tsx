...
export default function MyPostList({navigation, route}: Props) {
      ...
      headerRight: () =>
        deleteMode ? (
          <>
            <TouchableOpacity
              onPress={() => {
                if (myPostList.filter(p => p.isChecked).length > 0) {
  ...
  const handleBoardSearchComponent = (
    <TouchableHighlight
      ...
      underlayColor="#EEEEEE"
      onPress={() => {
        navigation.navigate('MyPostSearch');
      }}>
      <SearchIcon />
    </TouchableHighlight>
  );
  ...
  const fetchNextPage = async () => {
    ...
    let thisPagePostList: MyPostContentDto[] = await getMyPostList(
      currentPage + 1,
      sortBy,
    );
            ...
            {deleteMode && (
              <TouchableOpacity
                ...
                <Text
                  ...
                  {`${myPostList.filter(p => p.isChecked).length}/${
                    myPostList.length
                  }`}
                </Text>
