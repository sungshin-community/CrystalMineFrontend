...
export default function ScrapedPostList({navigation}: Props) {
  ...
  const moveToPost = (post: MyPostContentDto) => {
    if (deleteMode) {
      const tempList = myPostList.map(p =>
        p.postId === post.postId ? {...p, isChecked: !p.isChecked} : p,
      );
      ...
      headerRight: () =>
        deleteMode ? (
          <>
            <TouchableOpacity
              onPress={() => {
                if (myPostList.filter(c => c.isChecked).length > 0) {
  ...
  const handleBoardSearchComponent = (
    <TouchableHighlight
      ...
      underlayColor="#EEEEEE"
      onPress={() => {
        navigation.navigate('ScrapedPostSearch');
      }}>
      <SearchIcon />
    </TouchableHighlight>
  );
  ...
  const fetchNextPage = async () => {
    let thisPagePostList: MyPostContentDto[] = await getScrapedPostList(
      currentPage + 1,
    );
            ...
            {deleteMode && (
              <TouchableOpacity
                ...
                <Text
                  ...
                  {`${myPostList.filter(c => c.isChecked).length}/${
                    myPostList.length
                  }`}
                </Text>
      ...
      {deleteModalVisible && (
        <ModalBottom
          ...
          purpleButtonFunc={async () => {
            setIsLoading(true);
            await cancelScrapedPosts(
              myPostList.filter(p => p.isChecked).map(p => p.postId),
            );
