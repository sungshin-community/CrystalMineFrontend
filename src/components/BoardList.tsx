...
export default function BoardList({items}: Props) {
    return (
            <FlatList
                data={items}
                renderItem={({item})=><View style={{flexDirection: 'row', paddingVertical: 16, alignItems: 'center', backgroundColor: '#FFFFFF'}}>
                    {!item.isPinned ? <GrayPin style={{marginLeft: 25}} /> : item.isOfficial ? <OrangePin style={{marginLeft: 25}} /> : <PurplePin style={{marginLeft: 25}} />}
                    <Text style={{fontSize: 14, color: '#000000', marginLeft: 15}}>{item.title}</Text>
                    </View>}
            />
    )
}